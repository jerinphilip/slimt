...
inline bool endsWith(markup::StringRef &str, const CharType (&suffix)[Len]) {
  size_t offset = str.size - (Len - 1);
  return offset <= str.size &&
         std::memcmp(str.data + offset, suffix, Len - 1) == 0;
}

inline bool equalsCaseInsensitive(const char *lhs, const char *rhs,
                                  size_t len) {
...
template <size_t Len>
inline bool equalsCaseInsensitive(markup::StringRef &lhs,
                                  const char (&rhs)[Len]) {
  return lhs.size == Len - 1 && equalsCaseInsensitive(lhs.data, rhs, Len - 1);
}

template <typename CharType, size_t Len>
bool operator==(markup::StringRef const &str, const CharType (&str2)[Len]) {
...
Scanner::TokenType Scanner::scanAttribute() {
  ...
  switch (input_.peek()) {
    case '>':
      ...
      if (/*equalsCaseInsensitive(tagName_, "title") ||*/ equalsCaseInsensitive(
              tagName_, "script") ||
          equalsCaseInsensitive(tagName_, "style") ||
          equalsCaseInsensitive(tagName_, "textarea") ||
          equalsCaseInsensitive(tagName_, "iframe") ||
          equalsCaseInsensitive(tagName_, "noembed") ||
          equalsCaseInsensitive(tagName_, "noscript") ||
          equalsCaseInsensitive(tagName_, "noframes")) {
...
bool Scanner::resolveEntity(StringRef const &buffer, StringRef &decoded) const {
  static char lt = '<', gt = '>', amp = '&', quot = '"', apos = '\'',
              nbsp = ' ';
  ...
  if (buffer == "&nbsp;") {
    decoded = StringRef{&nbsp, 1};  // TODO: handle non-breaking spaces better
                                    // than just converting them to spaces
    return true;
  }
  return false;
}
...
Scanner::TokenType Scanner::scanSpecial() {
  ...
  while (true) {
    ...
    if (value_.data[value_.size - 1] == '>' &&
      ...
      if (!equalsCaseInsensitive(value_.data + tag_name_position, tagName_.data,
                                 tagName_.size))
        continue;
