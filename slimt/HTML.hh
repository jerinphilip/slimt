...
class HTML {
 public:
  ...
  struct Options {
    ...
    /// Technically almost all inline elements should be treated like this,
    /// except `<br>` maybe, But in practice it seems to be more effective to
 ...
 private:
  ...
  using AnnotatedText = slimt::AnnotatedText;

  /// Reconstructs HTML in `response.source` (passed as `in`) and makes a list
  /// `source_token_spans` that associates a `Span` with each subword in `in`.
  ...
  AnnotatedText restore_source(const AnnotatedText &in,
                               std::vector<SpanIterator> &source_token_spans);

  /// Inserts the HTML into `response.target` (passed as `in`) based on
  /// `target_token_spans`, which points to a `Span` for each token (subword) in
  /// `response.target`.
  AnnotatedText restore_target(
      const AnnotatedText &in,
      const std::vector<SpanIterator> &target_token_spans,
      const std::vector<HTML::TagStack> &target_token_tags);
  ...
  static void copy_tag_stack(
      const Response &response,
      const std::vector<std::vector<size_t>> &alignments,
      const std::vector<HTML::SpanIterator> &source_token_spans,
      std::vector<HTML::SpanIterator> &target_token_spans);

  static void annotate_tag_stack(
      const Response &response,
      const std::vector<SpanIterator> &target_token_spans,
  ...
  /// possible.
  void hard_align(const Response &response,
