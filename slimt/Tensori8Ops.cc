...
template <>
Tensor affine<i8xi8::kIntgemm>(Tensor& x, Tensor& W, Tensor& b, float a_quant,
...
template <>
Tensor dot<i8xi8::kIntgemm>(Tensor& x, Tensor& W, float a_quant, float b_quant,
                            const std::string& name) {
  // Naming is to simplify thinking with the intgemm API below.
  Tensor& A = x;  // NOLINT
  Tensor& B = W;  // NOLINT
...
// Ruy.
template <>
Tensor affine<i8xi8::kRuy>(Tensor& x, Tensor& W, Tensor& b, float a_quant,
                           float b_quant, const std::string& name) {
  Tensor& A = x;  // NOLINT
  Tensor& B = W;  // NOLINT
  Tensor& bias = b;
...
template <>
Tensor affine_with_select<i8xi8::kRuy>(Tensor& x, Tensor& W, Tensor& b,
                                       float a_quant, float b_quant,
                                       const std::vector<uint32_t>& indices,
                                       const std::string& name) {
  Tensor& A = x;  // NOLINT
...
template <>
Tensor dot<i8xi8::kRuy>(Tensor& x, Tensor& W, float a_quant, float b_quant,
                        const std::string& name) {
  Tensor& A = x;  // NOLINT
  Tensor& B = W;  // NOLINT
...
template <>
void PrepareBTransposed<i8xi8::kRuy>(const float* weights, int8_t* prepared,
                                     float quantization_multiplier, size_t cols,
                                     size_t rows) {
  detail::quantize(weights, quantization_multiplier, cols, rows, prepared);
}
template <>
void PrepareBQuantizedTransposed<i8xi8::kRuy>(const int8_t* input,
                                              int8_t* output, size_t rows,
                                              size_t cols) {
